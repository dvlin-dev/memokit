/**
 * [INPUT]: 截图 Buffer + 处理选项
 * [OUTPUT]: 处理后的图片 Buffer
 * [POS]: 图片后处理，格式转换、质量压缩、水印添加
 */

import { Injectable, Logger } from '@nestjs/common';
import sharp from 'sharp';
import type { ImageFormat, ProcessOptions, ProcessResult } from './screenshot.types';
import {
  FREE_TIER_WATERMARK,
  WATERMARK_FONT_SIZE,
  WATERMARK_PADDING,
...
export class ImageProcessor {
  ...
  private async addWatermark(
    ...
    try {
      ...
      // 计算水印位置（右下角）
      const x = Math.max(0, width - this.getWatermarkWidth() - WATERMARK_PADDING);
  ...
  private applyFormat(
    ...
    switch (format) {
      case 'png':
        ...
        // quality=100 → compressionLevel=0（不压缩，最快）
        // quality=1 → compressionLevel=9（最大压缩，最慢）
        return pipeline.png({
          compressionLevel: quality === 100 ? 0 : Math.round((100 - quality) / 11),
        });
   ...
   * 获取图片信息
   */
  async getInfo(input: Buffer): Promise<{ width: number; height: number; format: string }> {
