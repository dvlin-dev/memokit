...
export class QuotaService {
   ...
   * 确保用户有配额记录（如果不存在则创建）
   */
  async ensureExists(userId: string, tier: SubscriptionTier = DEFAULT_TIER): Promise<void> {
    const exists = await this.repository.exists(userId);
    if (!exists) {
   ...
   * 更新月度配额上限（套餐变更时调用）
   */
  async updateMonthlyLimit(userId: string, tier: SubscriptionTier): Promise<void> {
    const monthlyLimit = getMonthlyQuotaByTier(tier);
   ...
   * 检查并重置周期（如果需要）
   */
  async checkAndResetPeriodIfNeeded(userId: string): Promise<PeriodResetResult> {
    const quota = await this.repository.findByUserId(userId);
    if (!quota) {
   ...
   */
  async incrementConcurrent(userId: string, tier: SubscriptionTier): Promise<number> {
  ...
  async checkRateLimit(userId: string, tier: SubscriptionTier): Promise<void> {
    ...
    const result = await this.redis.checkRateLimit(userId, 60, config.ratePerMinute);
   ...
   * 检查功能是否可用
   */
  isFeatureAllowed(tier: SubscriptionTier, feature: keyof TierQuotaConfig['features']): boolean {
    const config = getTierConfig(tier);
